...
def cost(path, distances):
    ...
    if (not valid_path(path)):
        return sys.maxsize # instead of returning None or raising exception, I am deliberating making it a very high cost path so that algorithm avoids it.
    result = 0
...
def valid_path(path): # It should check if the path contains all the cities, right?
    for c in path:
        count = 0
        for c2 in path:
            if (c2 == c):
