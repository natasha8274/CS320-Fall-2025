...
class VertexEL:
    ...
    def __lt__(self, other):
        if not isinstance(other, VertexEL):
            raise Exception(
                f"VertexEL cannot be compared to {type(other)}"
            )
    ...
    def __hash__(self):
        """Allow vertex to be used as dict key or in set."""
...
class EdgeEL:
    ...
    def __lt__(self, other):
        if not isinstance(other, EdgeEL):
            raise Exception(
                f"EdgeEL cannot be compared to {type(other)}"
            )
        return self.name < other.name
    ...
    def __hash__(self):
        """Allow edge to be used as dict key or in set."""
        return hash(self.name)
...
class GraphEL:
    ...
    def __init__(self):
        self._vertices = {}
        self._edges = {}
        # OPTIMIZATION: Adjacency lists for O(1) access to incident edges
        self._adjacency = {}  # vertex.name -> list of edges
    ...
    def add_vertex(
        ...
        if vertex.name not in self._adjacency:
            self._adjacency[vertex.name] = []

    def rm_vertex(self, vertex):
        if vertex.name in self._vertices:
            # First, remove all incident edges from the graph and neighbor adjacency lists
            if vertex.name in self._adjacency:
                incident_edges = list(self._adjacency[vertex.name])  # Copy to avoid modification during iteration
                for edge in incident_edges:
                    self.rm_edge(edge)  # This handles removal from all adjacency lists

            # Then remove the vertex itself
            del self._vertices[vertex.name]
            if vertex.name in self._adjacency:
                del self._adjacency[vertex.name]
    ...
    def add_edge(self, edge):
        ...
        for v in edge.ends():
            if v.name not in self._vertices:
                self.add_vertex(v)
            # OPTIMIZATION: Build adjacency list - ensure key exists first
            if v.name not in self._adjacency:
                self._adjacency[v.name] = []
            if edge not in self._adjacency[v.name]:
                self._adjacency[v.name].append(edge)

    def rm_edge(self, edge):
        if edge.name in self._edges:
            del self._edges[edge.name]
            # OPTIMIZATION: Remove from adjacency lists
            for v in edge.ends():
                if v.name in self._adjacency and edge in self._adjacency[v.name]:
                    self._adjacency[v.name].remove(edge)
    ...
    def incident(self, v):
        """Returns edges incident to vertex v in O(deg(v)) time."""
        if v.name not in self._adjacency:
            return []
        return list(self._adjacency[v.name])

    def incoming(self, v):
        """Returns edges incoming to vertex v."""
        il = []
    ...
    def outgoing(self, v):
        """Returns edges outgoing from vertex v."""
        il = []
        for e in self.incident(v):
            if v == e.tail():
                il += [e]
    ...
    def adjacent(self, v):
        """Returns vertices adjacent to v in O(deg(v)) time."""
        av = []
    ...
    with open(file_path, "r") as file:
        for edge_index, line in enumerate(file):
            ...
            if len(parts) != 3:
                raise Exception(
                    (
                        f"Could not parse line {edge_index}: '{line.strip()}'. "
            ...
            if not isinstance(
                parts[0], int
            ) and not isinstance(parts[0], float):
                raise Exception(
                    (
                        f"Could not parse line {edge_index}: '{line.strip()}'. "
            ...
            edge_weight, v1_name, v2_name = (
                parts[0],
                parts[1],
                parts[2],
            )
            vertex1 = graph._vertices.get(
                v1_name, VertexEL(v1_name)
            )
